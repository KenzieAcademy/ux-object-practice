...
*/
/**
 * createPlant - Produces an object representing a plant.  It should have the following properties:
...
function addPlantToEstate(estate, plant) {
	plant.type !== 'rose'
		? plant.isPerennial && plant.amountOfSunNeeded <= 5
...
/* ------------------------------------------------
    ...
    They should each return a string, which is a legible english paragraph that nicely describes
    ...
    Try to have as little redundant code as possible! 
...
function describePlant(plant) {
	let description = `A ${plant.type}
    which has ${plant.leafColor} leaves that are ${plant.leafDescription}. The flowers are ${plant.flowerColor} ${plant.flowerDescription}.`;
	// Your Code Here!
...
function describeGarden(gardenName, listOfPlants) {
	...
	// Concatenating the description for each plant together into one big string.
...
function describeEstate(estate) {
	...
	let description = `${describeGarden('Rose Garden', estate.roseArbor)} ${describeGarden(
		'Perennial Garden',
 ...
 * The clone should have identical properties to the given plant, except for the color.
 *
...
function cloneRose(plant) {
	...
	let propertyArray = Object.keys(plant);
...
*/
{
	...
	console.log('-----Tests for Exercise Two-----');
	{
		...
		console.log('* Add a high sun perennial');
		...
		console.log('* Add a non-perennial');
