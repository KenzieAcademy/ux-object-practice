...
function describePlant(plant) {
  let description = `A ${plant.type} which has ${plant.leafDescription} that has ${plant.leafDescription}. The flowers are ${plant.flowerColor} ${plant.flowerDescription}`;
...
function describeGarden(gardenName, listOfPlants) {
  let description = `The ${gardenName} has ${listOfPlants.length} in it contains: `;
...
function cloneRose(plant) {
  ...
  for (let plantProperties of plantItems) {
    clone[plantProperties] = plant[plantProperties];
  }
...
function cloneAllTheRosesAndChangeTheirColors(estate) {
  ...
  for (let rose of estate.roseArbor) {
    cloneRoses.push(cloneRose(rose));
  }
  estate.roseArbor = estate.roseArbor.concat(cloneRoses);
  // Your Code Here!
  // for each rose...
